...
The subsegmentation workflow is now ready for use and closely follows the steps for running fMRIPrep in Nipoppy. This pipeline is designed to extract subnuclei volumes from subcortical regions such as the thalamus, hippocampus, brainstem (and more!) based on FreeSurfer 7 output.  
**It is essential that FreeSurfer output is already available before starting this step**, as the subsegmentation pipeline directly depends on the segmentations previously generated by FreeSurfer.
...
To get started, download the required files from Zenodo. Together, these files allow Nipoppy to recognize, configure, run, and monitor the custom container image as part of its structured processing framework. You will need the following:
...
Place these files in a subfolder under `code/`, for example:

```
<dataset_root>/code/subsegmentations/
```

### Setting up configuration

Make sure the new pipeline is listed under `PROC_PIPELINES` in your global config file:
...
The name here (`subseg`) must match the folder name under `code/`.
...
```bash
nipoppy run --pipeline subseg <dataset_root>
```

To process only a specific participant or session, use the optional flags:

```bash
nipoppy run --pipeline subseg \
...
```bash
nipoppy track --pipeline subseg <dataset_root>
```
